...
def should_offer_deep_analysis(user_history: List[Dict]) -> bool:
    ...
    user_messages = [msg["content"] for msg in user_history if msg["role"] == "user"]
    if len(user_messages) < 3:
        return False
    
    last_user_messages = user_messages[-3:]
...
def gpt_decide(uid: int, text_in: str, st: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        system_prompt = f"""
        Ты — Алекс, эмпатичный и мудрый наставник для трейдеров. Твоя главная цель — помочь разобраться в проблемах, но сначала установить глубокий контакт.
...
@app.get("/status")
def status():
    return jsonify({"ok": True, "time": datetime.utcnow().isoformat(), "version": "2025-09-01-empathic-v2"})
