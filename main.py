...
# напоминалки
IDLE_REMINDER_MIN = int(os.getenv("IDLE_REMINDER_MIN", "60"))
SESSION_CONTINUE_THRESHOLD_MIN = int(os.getenv("SESSION_CONTINUE_THRESHOLD_MIN", "60"))
...
def gpt_decide(uid: int, text_in: str, st: Dict[str, Any]) -> Dict[str, Any]:
    """
    НЕ даёт советы. Вытягивает 3 сигнала (trigger/action/cost),
    формулирует проблему и просит подтверждение.
    """
    fallback = {
        "mode": "calibration",
        "next_step": st["step"],
        "intent": st["intent"],
        "response_text": "Хочу точнее зафиксировать момент. Что именно было триггером?",
        "store": {},
        "is_structural": False,
    ...
    system_prompt = f"""
Ты — коуч Алекс. Коротко, по-человечески, без советов.
Цель — калибровка (не техника):
1) вытащить три сигнала: trigger (что запускает), action (что делаешь), cost (чем это обходится);
2) собрать короткую формулировку проблемы (1 предложение);
3) если ясно — предложить подтвердить формулировку.

Нельзя давать советы/метод-названия.
Ответ строго JSON:
{{
  "mode": "calibration",
  "response_text": "строка на «{style}»",
  "store": {{
     "calibrated_delta": {{
        "problem_text": "<или пусто>",
        "signals": {{"trigger":"...", "action":"...", "cost":"..."}},
        "rounds_inc": 1
     }},
...
def handle_text_message(uid: int, text_in: str, original_message=None):
    ...
    # если пауза > threshold — спросим о продолжении
    try:
    ...
    resp_bridge = soft_bridge(resp)

    # применяем дельты калибровки
    merged = st["data"].copy()
    ...
    # если готово — подтвердить проблему
    ...
    if original_message:
        bot.reply_to(original_message, resp_bridge, reply_markup=MAIN_MENU)
    else:
        bot.send_message(uid, resp_bridge, reply_markup=MAIN_MENU)
...
def handle_structural(uid: int, text_in: str, st: Dict[str, Any]):
    # переформулировка на лету — назад к калибровке
