...
def detect_trading_patterns(text: str) -> List[str]:
    ...
    patterns = {
        ...
        "doubt": ["сомневаться", "не уверен", "кажется рано", "кажется поздно"]
    }
...
def generate_concrete_question(pattern: str) -> str:
    """Генерируем конкретные вопросы вместо шаблонных"""
    questions = {
        "remove_stop": "На сколько пунктов против тебя пошла цена перед тем как ты убрал стоп?",
        "early_close": "Какой был размер прибыли когда ты закрылся? И какой мог бы быть TP?",
        "averaging": "В какой момент ты решил усреднять? Что происходило на графике?",
        "doubt": "Что конкретно изменилось в твоём восприятии сетапа после входа в сделку?",
        "default": "Опиши подробно последний случай, когда это произошло: по какому активу и что именно случилось?"
    }
    return questions.get(pattern, questions["default"])
def should_ask_follow_up(state: Dict, last_question: str) -> bool:
    """Определяем, стоит ли задавать уточняющий вопрос"""
    history = state.get("data", {}).get("history", [])
    
    # Не спрашиваем одно и то же дважды
    last_questions = [msg["content"] for msg in history[-4:] 
                     if msg.get("role") == "assistant" and "?" in msg["content"]]
    
    if last_question in last_questions:
        return False
        
    # Проверяем, не отвечал ли уже пользователь на этот вопрос
    user_responses = [msg["content"] for msg in history[-4:]
                     if msg.get("role") == "user"]
    
    question_keywords = {
        "как долго": ["долго", "время", "период"],
        "ситуации": ["ситуация", "случай", "момент"]
    }
    
    for keyword, variants in question_keywords.items():
        if any(variant in last_question for variant in variants):
            if any(variant in response for response in user_responses for variant in variants):
                return False
...
def gpt_decide(uid: int, text_in: str, st: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        6. НИКОГДА не начинай ответ с шаблонных фраз: 'Понимаю, это...', 'Я понимаю, что...', 'Это может быть...'
        7. Начинай ответ сразу с сути, без воды
        8. Избегай повторяющихся вопросов. Если уже спрашивал о чем-то, не спрашивай снова.
        9. Задавай конкретные вопросы о последнем случае, а не общие.
        ...
        if not should_ask_follow_up(st, dec["response_text"]):
            main_pattern = patterns[0] if patterns else "default"
            dec["response_text"] = generate_concrete_question(main_pattern)
...
def handle_text_message(uid: int, text: str, original_message=None):
    ...
    if st["intent"] == INTENT_GREET and st["step"] == STEP_ASK_STYLE:
        if text.lower() in ("ты", "вы"):
            ...
            bot.send_message(uid, f"Принято ({text}). Расскажи, что сейчас происходит в твоей торговле? Какие вызовы или сложности заметил?", reply_markup=MAIN_MENU)
        else:
...
@app.get("/status")
def status():
    return jsonify({"ok": True, "time": datetime.utcnow().isoformat(), "version": "2025-09-01-mentor-v3"})
